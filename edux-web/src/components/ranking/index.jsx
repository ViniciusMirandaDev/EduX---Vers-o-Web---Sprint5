...
class Ranking extends Component {
        ...
        return usuarios.map((usuario, index) => {
            return (
                <tr key={index}>
                    ...
                    {index === 0 && <td>{usuario.usuario.nome}</td>}
                    {index === 1 && <td>{usuario.usuario.nome}</td>}
                    {index === 2 && <td>{usuario.usuario.nome}</td>}
                    {index > 2 && <td>{usuario.usuario.nome}</td>}
    ...
    render() {
        ...
        return users.length > 0
            ? (
                <>
                    <Titulo titulo={`Bem vindo à página de ranking, ${nomeUsuario}!`} chamada="veja sua posição entre as turmas no ranking abaixo!" />
